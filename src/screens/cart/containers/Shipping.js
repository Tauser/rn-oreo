...
import {StyleSheet, ScrollView, View, Switch, ActivityIndicator} from 'react-native';
...
import {updateAddress, changeChosenMethod, changeIsDifferentAddress} from 'src/modules/checkout/actions';
import {
  shippingSelector,
  billingSelector,
...
function Shipping(props) {
  ...
  const saveAddress = (customer) => {
    ...
    if (!isDifferentAddressSelector || shipping.equals(fromJS(shippingAddressInit))) {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <Container>
          ...
          <ShippingForm
            ...
            data={billing}
            type="billing"
            onChange={(key, value) => dispatch(updateAddress({billing: {[key]: value}}))}
          />
          ...
          {isDifferentAddress ? (
            <>
              ...
              <ShippingForm
                ...
                data={shipping}
                type="shipping"
                onChange={(key, value) => dispatch(updateAddress({shipping: {[key]: value}}))}
              />
            </>
          ) : null}
          {errors.get('shipping_lines') ? <Text style={{color: red}}>{errors.get('shipping_lines')}</Text> : null}
          {loadingMethod ? (
            <View>
              <ActivityIndicator size="small"/>
            </View>
            ...
            <View key={index} style={styles.viewMethod}>
              <Heading
                title={method.getIn(['store','store_name']) ? t('cart:text_shipping_store', {name: method.getIn(['store','store_name'])}) : t('cart:text_shipping')}
