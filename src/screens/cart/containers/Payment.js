...
import {paymentGatewaysSelector, currencySelector} from 'src/modules/common/selectors';
...
import {selectPaymentMethod, changeCustomerNote, checkout} from 'src/modules/checkout/actions';
...
class Payment extends React.Component {
  ...
  render() {
    const {backStep, nextStep, shipping, customerNote, paymentGateway, paymentMethod, t, dispatch, totals} = this.props;
      ...
      <View style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Container>
            ...
            />
            {errors.get('payment_method') ? <Text style={{color: red}}>{errors.get('payment_method')}</Text> : null}
            <PaymentMethod nextStep={nextStep} lists={paymentGateway} selected={paymentMethod} selectMethod={this.selectMethod}/>
            <PaymentForm
              shipping={shipping}
        ...
        <Modal visible={isModal} setModalVisible={value => this.setState({isModal: value})} ratioHeight={0.9}>
          {message ? (
            <WebView
              originWhitelist={['*']}
              source={{html}}
